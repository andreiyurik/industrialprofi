...
JOURNALS = {
  "ivan.petrov@example.com" => [
    ...
    { title: "Разобрался с группами допуска", days: 20,
      body: "Перечитал статью про II группу. Записал себе порядок действий перед допуском к работам — теперь не путаюсь." }
  ],
  "maria.kuznetsova@example.com" => [
    { title: "Прочитала первую схему ФСА", days: 5,
      body: "Сначала пугали обозначения, но по статье разложила P&ID на контуры. Нашла контур регулирования уровня — позиции совпали с таблицей." },
...
FEEDBACKS = [
  ...
  { email: "pavel.novikov@example.com",   days: 4,  read: true,  url: nil,
    body: "Нашёл опечатку в статье про калибровку, отправил правку через «предложить изменение». Как понять, что её приняли?" },
...
POSTS = [
  { slug: "zapusk-poiska-po-urokam", days: 4,
    title: "Запустили поиск по статьям",
    excerpt: "Полнотекстовый поиск по всем статьям платформы — ищите тему, термин или номер документа.",
    body: "<p>Добавили полнотекстовый поиск по всем опубликованным статьям — на SQLite FTS5, без единого нового сервиса. Ищите тему, термин или номер документа, найденное подсвечивается прямо в сниппете.</p><p>Быстрое окно поиска открывается с любой страницы — значок в шапке, Ctrl+K или «/».</p>" },
  ...
  { slug: "chistka-ssylok-pered-zapuskom", days: 2,
    ...
    body: "<p>Прошлись по всем профессиям и заменили ссылки на consultant.ru/garant.ru (требуют платную регистрацию) на бесплатные официальные источники — pravo.gov.ru, сайты издающих ведомств, protect.gost.ru.</p><p>Заодно убрали случайные повторы и слабые ссылки, которые просто пересказывали статью.</p>" }
].freeze
POSTS.each do |p|
