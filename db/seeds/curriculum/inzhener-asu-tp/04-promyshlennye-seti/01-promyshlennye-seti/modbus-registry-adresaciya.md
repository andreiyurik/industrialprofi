...
Вспомни прошлый урок: [RS-485](/ru/lessons/promyshlennye-seti-rs485-master-slave) — дорога, Modbus — язык. Modbus задаёт, **как именно** master формирует запрос, а slave отвечает: какие байты, в каком порядке, что они значат. Он простой, открытый и потому победил и он один из самых популярных и еще долго им останется.
